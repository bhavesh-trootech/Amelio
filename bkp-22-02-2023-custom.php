		...
		<p>
			<span class="first last">
				<label>
					...
					<span class="wpcf7-list-item-label">I have read and agree to amelio’s <a class="amelio-privacy" href="#">Privacy Policy</a>.</span>
				</label>
			</span>
		</p>
      ...
      jQuery('#fname').donetyping(function(){
        var fname = jQuery('#fname').val();
          if(fname == ""){
              ...
              jQuery('#fnameerror').text("Please enter valid name.");
              return false;
            }else{
       ...
       jQuery('#lname').donetyping(function(){
        var lname = jQuery('#lname').val();
          if(lname == ""){
              ...
              jQuery('#lnameerror').text("Please enter valid name.");
              return false;
            }else{
...
var isValidExperience = experienceChecked > 0;
 ...
 if(!validateName.test(fname)){
	jQuery('#fname').addClass('require');
    jQuery('#fnameerror').text('Please enter the First name');
    jQuery("#fname").focus();
    valid = false;
} else if(!validateName.test(lname)){
    jQuery('#fname').removeClass('require');
    jQuery('#fnameerror').text('');
    jQuery('#lname').addClass('require');
    jQuery('#lnameerror').text('Please enter the Last name');
    jQuery("#lname").focus();
    valid = false;
	...
	if(!empty($therapistusrid)){
		...
		if ( $exists ) {
			...
			if(!empty($userlog)){
				...
				$to ='gayatri.patel@trooinbound.com,'.$emailid;
				$subject = 'Amelio Therapist Login Details';
				$body =' <h4 style="font-size: 16px;margin: 10px 0;">Hi <strong>,'.$fname.' '.$lname.'</strong></h4>';
				$body .='<p><b>Please Find Your Login Details Below :<b></p>';
				$body .='<p><b>Email Id :</b>'.$emailid.'</p>';
				$body .='<p><b>Password :</b>'.$random_password.'</p>';
				$body .='Thank You';
